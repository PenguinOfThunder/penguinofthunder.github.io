...
export function datetime_format(input, ...args) {
  ...
  const { locales = page.lang, ...options } = params;
  const d =
...
export function filter_array(input, ...param) {
  // const { page, eleventy } = this;
  const { except = "" } = Object.fromEntries(param || []);
  const exceptArray = except.split(",");
  return (input || []).filter((item) => exceptArray && !exceptArray.includes(item));
}
